...
│                     执行层（执行实现）                             │
├─────────────────────────────────────────────────────────────────┤
│    pm     │   dev    │ backend-dev │ frontend-dev               │
│ (执行PM)  │ (通用执行) │ (后端工程师) │ (前端工程师)            │
└─────────────────────────────────────────────────────────────────┘
                              │
                              ▼
┌─────────────────────────────────────────────────────────────────┐
...
| **代码实现** | dev / backend-dev / frontend-dev | ❌ 不得自行引入新架构或修改设计 |
| **质量把关** | qa | ❌ 不修代码，只反馈问题 |
...
| **Dev** | [dev.md](dev.md) | 通用执行工程师。实现已明确设计的功能。 | ❌ 不得自行引入新架构或修改设计 |
...
用户: Git commit 只标记 [T016]
pm: ❌ Commit 必须包含 LWP 和 T 编号：[LWP-1][T016] feat: ...
```
...
- ✅ dev：弃用，拆分为 backend-dev 和 frontend-dev
