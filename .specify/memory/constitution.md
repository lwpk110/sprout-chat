...
**原则**：所有文档必须遵循统一的文档规范标准。

**文档规范要求**：
- **目录结构**：文档必须放在正确的分类目录下（`adr/`, `technical/`, `reports/`, `development/`, `deployment/`）
- **命名规范**：文件名使用小写字母和连字符，包含描述性信息
- **元数据完整**：所有文档必须在顶部包含 YAML 元数据（title, type, status, version, created, updated, author）
- **生命周期管理**：文档状态必须明确（draft/review/approved/deprecated/archived）
- **索引维护**：新增或删除文档时必须更新 `docs/README.md` 索引

**不可违反的规则**：
- ❌ 禁止在项目根目录或 `docs/` 根目录散落文档
- ❌ 禁止使用不规范文件名（大写、空格、中文）
- ❌ 禁止缺少元数据或元数据不完整
- ❌ 禁止文档过时未标记（deprecated）
- ✅ 必须使用文档模板（`docs/.templates/`）
- ✅ 必须在正确的分类目录下创建文档
- ✅ 必须更新文档索引

**文档分类标准**：
- **架构决策**（ADR）：`docs/adr/` - 技术决策记录
- **技术文档**：`docs/technical/` - API、数据库、AI 配置
- **开发文档**：`docs/development/` - 开发指南、最佳实践
- **报告**：`docs/reports/` - 阶段报告、迭代报告、最终报告
- **部署文档**：`docs/deployment/` - Docker、生产环境
- **归档文档**：`docs/archive/` - 过期但保留的文档

**理由**：统一的文档规范防止文档混乱，提高可维护性，支持自动化文档生成和检查，便于新人快速上手和团队协作。

**详细规范**：参见 `docs/.templates/document-standard.md`
...
**规则**：所有文档必须遵循文档规范标准，保持格式和结构一致。

**一致性要求**：
1. **元数据完整**：所有文档必须包含完整的 YAML 元数据
2. **目录规范**：文档必须放在正确的分类目录下
3. **命名规范**：文件名使用小写字母和连字符
4. **状态明确**：文档状态必须准确反映当前状态
5. **索引同步**：文档索引必须与实际文档同步

**检查清单**：
- [ ] 文件名符合命名规范（小写+连字符）
- [ ] 包含完整元数据（title, type, status, version, created, updated, author）
- [ ] 文档放在正确的目录下
- [ ] 文档状态正确（draft/review/approved/deprecated/archived）
- [ ] 已更新 `docs/README.md` 索引

**违规处理**：
- 文档缺少元数据 → 禁止合并，补充元数据
- 文档放错目录 → 移动到正确目录
- 文档过时未标记 → 标记为 deprecated 或归档
- 索引未更新 → 更新索引后再合并

**理由**：统一的文档规范确保文档的可维护性、可查找性和可读性，防止文档散落和混乱。
...
**版本**：1.1.0 | **批准日期**：2025-01-12 | **最后修订**：2026-01-15
